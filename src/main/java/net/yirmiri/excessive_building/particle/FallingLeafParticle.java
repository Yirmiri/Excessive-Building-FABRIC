...
public class FallingLeafParticle extends SpriteBillboardParticle {
    private static final float FLOAT = 0.0025F;
    private static final int INT = 300;
    private static final int INT1 = 300;
    private static final float FLOAT1 = 0.25F;
    ...
    public void tick() {
        ...
        if (!this.dead) {
            ...
            double d = Math.cos(Math.toRadians((double) (this.aFloat1 * 60.0F))) * 2.0 * Math.pow((double) g, 1.25);
            double e = Math.sin(Math.toRadians((double) (this.aFloat1 * 60.0F))) * 2.0 * Math.pow((double) g, 1.25);
